...
declare @end_date date = '2016-10-31'
declare @fy_end_date date = '2015-10-31'

declare @EndDate date = @end_date

declare @VetDate date = @end_date

declare @PWDDate date = @end_date

declare @FYStart date = @fy_end_date
...
from 
	...
	left join technical_job tt on tt.[Job Family Code] = wd.[Job Family Code]
	left join hpw_veteran tv on tv.[Employee ID] = wd.[Worker ID] and tv.[Report Date] = @VetDate
	left join hpw_w_disability tpwd on tpwd.[Employee ID] = wd.[Worker ID] and	tpwd.[Report Date] = @PWDDate
where 
